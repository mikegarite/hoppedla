...
function ism_shortcode_update(tab){
    ...
    if (tab=='shortcode_locker'){
    	var str = "[indeed-social-locker ";
    } else {
    	var str = "[indeed-social-media ";
    }
    str += "sm_list='"+jQuery('#sm_items').val()+"' ";
    str += "sm_template='"+the_template+"' ";
    str += "sm_list_align='"+jQuery('#list_align_type').val()+"' ";
    if (jQuery('#display_counts').is(':checked')){
    	str += "sm_display_counts='true' ";
    } else {
    	str += "sm_display_counts='false' ";
    }
    if (jQuery('#display_full_name').is(':checked')){
    	str += "sm_display_full_name='true' ";
    } else {
    	str += "sm_display_full_name='false' ";
    }
    ...
    if (tab=='shortcode_locker'){
    	str += "unlock_type=" + jQuery('#unlock_type').val() + " ";
        ...
        //targeting
        if (jQuery("#reg_users_enabled").val()==1){
        	//show/hide to registered users
        	str += "reg_users='" + jQuery('#reg_users').val() + "' ";
        }
        if (jQuery("#user_commented_enable").val()==1){
        	//show/hide to users that commented
        	str += "user_commented='" + jQuery('#user_commented').val() + "' ";
        }
        if (jQuery("#user_roles_enable").val()==1){
        	//show/hide to user roles
        	str += "user_roles='" + jQuery('#user_roles').val() + "' ";
        	if (jQuery("#user_roles_val").val()){
        		str += "user_roles_val='" + jQuery("#user_roles_val").val() + "' ";
        	} else {
        		str += "user_roles_val='' ";
        	}        	
        }
        if (jQuery("#referer_visits_enable").val()==1){
        	//show/hide to referers
        	str += "referer_visits='" + jQuery('#referer_visits').val() + "' ";
        	var refs = new Array();
        	if (jQuery("#referer_google").is(":checked")){
        		refs.push('google');
        	}
        	if (jQuery("#referer_yahoo").is(":checked")){
        		refs.push('yahoo');
        	}
        	if (jQuery("#referer_bing").is(":checked")){
        		refs.push('bing');
        	}
        	var refs_str = refs.join(',');
        	if (jQuery("#custom_refs").val()){
        		if (refs_str!=''){
        			refs_str += ',';
        		}
        		refs_str += jQuery("#custom_refs").val();
        	}
        	
        	str += "referer_values='" + refs_str + "' ";
        }        
		...
		if(jQuery('#delay_lk').val()==1){
        	str += "delay_lk=1 ";
        	if(jQuery('#ism_delay_lk').val()!='') str += "sm_delay_lk="+jQuery('#ism_delay_lk').val()+" ";
        }        
...
function check_and_h_sh(from, target){
	if (jQuery(from).is(":checked")){
		jQuery(target).val('show');
	} else {
		jQuery(target).val('hide');
	}
}
...
function ism_vc_write_data_from_multiselect(f, t){
	var v = jQuery(f).val();
	var s = v.join(",");
	jQuery(t).val(s);
}

function ism_vc_write_ref(){
	var refs = new Array();
	if (jQuery("#referer_google").is(":checked")){
		refs.push('google');
	}
	...
	var refs_str = refs.join(',');
	if (jQuery("#ism_vc_custom_refs").val()){
		if (refs_str!=''){
			refs_str += ',';
		}
		refs_str += jQuery("#ism_vc_custom_refs").val();
	}
	jQuery("#referer_values").val(refs_str);
}

function ism_select_sm_icon(s){
	var p = jQuery(s).parent().parent();
	if (jQuery(s).is(":checked")){		
		jQuery(p).addClass('ism_item_selected');
	} else {
		jQuery(p).removeClass('ism_item_selected');
	}
}

function ism_check_change_class_shortlocker_item(c, t){
	if (jQuery(c).is(':checked')){
		jQuery(t).addClass('ism_locker_targeting_active_settings');
		jQuery(t).removeClass('ism_locker_targeting_inactive_settings');
	} else {
		jQuery(t).addClass('ism_locker_targeting_inactive_settings');
		jQuery(t).removeClass('ism_locker_targeting_active_settings');
	}
}