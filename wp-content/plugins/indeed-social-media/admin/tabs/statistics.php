...
<?php
	...
	$ism_list = ism_return_general_labels_sm('', TRUE, '', FALSE);
?>
...
Also, a list of the most share pages are showing up.
	...
	if (isset($totals) && count($totals)>0){
		...
		foreach ($totals as $key=>$value){
			if (!empty($ism_list[$key]['font']) && $ism_list[$key]['font']=='socicon'){
				echo '<div class="ism_item ism_box_'.$key.'">'
						.'<i class="ism-sc-icon ism-socicon-' . $ism_list[$key]['long_key'] . '"></i>'
					...
					<select name="sm_type" onChange="jQuery('#ism_form').submit();" style="min-width:160px; margin-left:10px;">
						...
						<?php 
							foreach ($ism_list as $k=>$v){
								...
								?>
								<option value="<?php echo $v['long_key'];?>" <?php echo $selected;?>><?php echo $v['label'];?></option>
								<?php 	
							}
						?>
					</select>						
					...
					<tr style="background-color: #fff;">
						...
						<?
							foreach ($list_ism_arr as $k=>$v){
								if (!empty($ism_list[$k]['font']) && $ism_list[$k]['font']=='socicon'){
									echo '<td><i class="ism-sc-icon ism-socicon-' . $ism_list[$k]['long_key'] . '"></i></td>';
								} else {
									echo '<td><i class="fa-ism fa-'.$ism_list[$k]['long_key'].'-ism"></i></td>';
								}
					...
					foreach ($data as $obj){
						echo '<tr>';
							...
							foreach ($new_arr as $list_ism_arr){
								echo '<tr>';
								foreach ($list_ism_arr as $k=>$v){
									$the_key = $ism_list[$k]['long_key'];
									echo '<td>' . $obj->$the_key . '</td>';
								}
								echo '</tr>';
							}
							echo '</table></td>';
