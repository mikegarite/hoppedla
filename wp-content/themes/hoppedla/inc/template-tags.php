 ...
 */
function progression_menu_category_nav() {
	$term 			= get_queried_object();
	$term_all 		= ( $term->parent ? $term->parent : $term->term_id );
	$term_children 	= get_term_children( $term->term_id, 'beer_type' );
	...
	return sprintf(
		...
		// URL to the term archive
		get_term_link( $term_all, 'beer_type' ),

		// link text
		__( 'All', 'progression' ) )

		// append the subcategories
		.wp_list_categories( 'taxonomy=beer_type&echo=0&show_count=0&title_li=&show_option_none=&child_of=' . $term_all );
}