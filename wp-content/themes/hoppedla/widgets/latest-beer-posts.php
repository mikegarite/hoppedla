<?php
add_action('widgets_init', 'pyre_homepage_menu_load_widgets');

function pyre_homepage_menu_load_widgets()
{
	register_widget('Pyre_Latest_Menu_Media_Widget');
}

class Pyre_Latest_Menu_Media_Widget extends WP_Widget {
	
	function Pyre_Latest_Menu_Media_Widget()
	{
		$widget_ops = array('classname' => 'pyre_homepage_media-menu', 'description' => 'Latest Beer Posts');

		$control_ops = array('id_base' => 'pyre_homepage_media-widget-menu');
		
		add_action( 'load-widgets.php', array(&$this, 'my_custom_load') );
		
		
		$this->WP_Widget('pyre_homepage_media-widget-menu', 'Home: Beer Posts ', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
	 ...
	 ?>
		...
		<div class="<?php echo esc_attr(  $args['widget_id'] ); ?> portfolio-posts-home<?php if($checkbox_pro): ?> light-fonts-pro<?php endif; ?>" <?php if($widget_bg): ?>style="background-color:<?php echo esc_attr( $widget_bg ); ?>;"<?php endif; ?>>
			<div class="width-container">
				...
				<?php if($summary_text): ?>
						<div class="summary-portfolio-pro"><h4><?php echo esc_attr( $summary_text ); ?></h4></div>
				<?php endif; ?>
	...
	function form($instance)
	{
		...
		$defaults = array('title' => 'Featured Beers', 'summary_text' => '', 'posts' => 3, 'columns' => 3, 'portfolioslug' => '', 'link_text' => '', 'link_link' => '', 'widget_bg' => '#eeeeee', 'widget_bg_img' => '', 'checkbox_pro' => '1', 'link_icon' => '');
		...
		<p>
			<label for="<?php echo $this->get_field_id('portfolioslug'); ?>"><?php _e( 'Beer Category Slug<br>(Leave blank to pull all posts)', 'progression' ); ?>:</label>
